...
def _process_triples_iterable(triples_iterable, entity_id_to_idx, relation_id_to_idx, head_applied, tail_applied):
    processed_sp = {}
    processed_po = {}
    longest = 0
    ...
    for s_raw, p_raw, o_raw in triples_iterable:
        ...
        if (S, P) in processed_sp:
            idx_O = None
            for idx, cand in enumerate(filtered_tail_candidates_idx):
                if cand == O:
                    idx_O = idx
                    break
            if idx_O is not None:
                processed_sp[(S, P)]["candidates"].append(O)
                processed_sp[(S, P)]["rules"].append(copy.copy(rules_tails[idx_O]))
        else:
            processed_sp[(S, P)] = {
                "candidates": copy.deepcopy(filtered_tail_candidates_idx),
                "rules": copy.deepcopy(rules_tails),
            }

        if (P, O) in processed_po:
            idx_S = None
            for idx, cand in enumerate(filtered_head_candidates_idx):
                if cand == S:
                    idx_S = idx
                    break
            if idx_S is not None:
                processed_po[(P, O)]["candidates"].append(S)
                processed_po[(P, O)]["rules"].append(copy.copy(rules_heads[idx_S]))
        else:
            processed_po[(P, O)] = {
                "candidates": copy.deepcopy(filtered_head_candidates_idx),
                "rules": copy.deepcopy(rules_heads),
            }

    return processed_sp, processed_po, longest
...
def _process_relation_file(task):
    ...
    processed_sp, processed_po, longest = _process_triples_iterable(
        triples,
...
def _split_target_by_relation(target_file, entity_id_to_idx, relation_id_to_idx, tmp_dir):
    file_handles = {}
    relation_files = {}
    try:
        ...
        for f in file_handles.values():
            f.close()
    return [(p_raw, relation_files[p_raw]) for p_raw in relation_files]


def preprocess_candidates_from_applied(target_file, entity_ids, relation_ids, applied_rules, num_workers=1):
    ...
    try:
        ...
        with ctx.Pool(
            ...
            for part_sp, part_po, part_longest in tqdm(
...
def main():
    ...
    with open(os.path.join(args.save_dir, f"processed_po_{args.split}.pkl"), "wb") as f:
        pickle.dump(processed_po, f, protocol=pickle.HIGHEST_PROTOCOL)

    print(f"Saved processed_sp/processed_po for split={args.split} in {args.save_dir}")
