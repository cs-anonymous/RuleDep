...
RUN_SUFFIX="${RUN_SUFFIX:-true_rr_full_20260501}"
GPU="${GPU:-0}"
MULTIPROCESS="${MULTIPROCESS:-2}"
LOG_DIR="${LOG_DIR:-${ROOT_DIR}/logs/true_official_per_query_rr_full}"

if [ "$#" -eq 0 ]; then
    echo "Usage: $0 DATASET[:EXPERIMENT] ..." >&2
    exit 2
fi
...
run_one() {
    ...
    local log_file="${LOG_DIR}/${dataset}_${experiment}_gpu${GPU}.log"
    mkdir -p "${exp_dir}"

    echo "[$(date '+%F %T')] START dataset=${dataset} experiment=${experiment} gpu=${GPU} suffix=${RUN_SUFFIX}" | tee -a "${LOG_DIR}/master_gpu${GPU}.log"
    (
        cd "${ROOT_DIR}"
    ...
    echo "[$(date '+%F %T')] END dataset=${dataset} experiment=${experiment} gpu=${GPU} status=${status}" | tee -a "${LOG_DIR}/master_gpu${GPU}.log"
    return "${status}"
}
...
for item in "$@"; do
    case "${item}" in
        ...
        hetionet)
            ...
            ;;
        *)
            echo "Unknown dataset item: ${item}" >&2
            exit 2
            ;;
    esac
done
