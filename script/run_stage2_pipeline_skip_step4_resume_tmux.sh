...
# As requested: skip step4 for YAGO3-10 + hetionet; only run hetionet step5-stage1.
ALL_DATASETS=(
  "KG20C"
  "codex-m"
  "WN18RR"
...
run_stage2_for_dataset() {
  ...
  if [ ! -d "$stage1_dir" ]; then
    log "WARN  ds=$ds stage1_state missing at $stage1_dir, continue"
    FAIL_COUNT=$((FAIL_COUNT + 1))
    return 0
  fi
...
main() {
  ...
  log "SKIP step4 for YAGO3-10 and hetionet (as requested)"

  log "===== PREP START: hetionet stage1-only ====="
  run_hetionet_stage1_only
  log "===== PREP END: hetionet stage1-only ====="
