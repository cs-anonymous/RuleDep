...
def build_overall_results_md(
    ...
    lines.append("")
    lines.append("本表汇总当前仓库里所有已完成实验的 `test` 指标，时间统一尽量用秒表示。")
    lines.append("")
    lines.append("相关表格：")
    lines.append("")
...
def build_relation_analysis_md(
    ...
    lines.append("")
    lines.append("本节关注一个更细粒度的问题：当我们在数据集级别选定最优 aggregation 配置之后，dependency 究竟改善了哪些 relation，又在哪些 relation 上带来了负迁移。")
    lines.append("")
    lines.append("相关表格：")
    lines.append("")
    ...
    lines.append("")
    lines.append("各数据集使用的数据集级最优配置如下：")
    lines.append("")
    ...
    lines.append("")
    lines.append(f"在全部 `{total_relations}` 个 relation 中，`{pos_count}` 个 relation 的相对增益超过 `3%`，`{neu_count}` 个 relation 落在 `0%-3%` 的稳定提升区间，另有 `{neg_count}` 个 relation 出现负迁移。整体上，dependency 的收益并不是均匀分布的，而更像是集中出现在一批“baseline 尚未饱和但结构信号较强”的 relation 上。")
    lines.append("")
    ...
    if positive_group and negative_group:
        ...
        lines.append("")
        lines.append(
            f"正增益 relation 的平均 stage1 MRR 为 `{fmt_float_short(positive_group['avg_baseline_stage1_mrr'])}`，低于负增益 relation 的 `{fmt_float_short(negative_group['avg_baseline_stage1_mrr'])}`；"
            f"而其平均 dependency density 为 `{fmt_float_short(positive_group['avg_dep_per_rule'])}`，高于负增益 relation 的 `{fmt_float_short(negative_group['avg_dep_per_rule'])}`。"
            " 这说明 dependency 更容易帮助那些仍有提升空间、且规则交互相对更密的 relation。"
        )
        lines.append("")
        lines.append("进一步看中位数统计，正增益 relation 的典型规模特征如下：")
        lines.append("")
        lines.append(f"- `train triples` 中位数：`{_median_text(positive_rows, 'train_triple_count')}`，负增益为 `{_median_text(negative_rows, 'train_triple_count')}`")
        lines.append(f"- `test triples` 中位数：`{_median_text(positive_rows, 'test_triple_count')}`，负增益为 `{_median_text(negative_rows, 'test_triple_count')}`")
        lines.append(f"- `#rules` 中位数：`{_median_text(positive_rows, 'num_relation_rules')}`，负增益为 `{_median_text(negative_rows, 'num_relation_rules')}`")
        lines.append(f"- `#dependencies` 中位数：`{_median_text(positive_rows, 'num_relation_dependencies')}`，负增益为 `{_median_text(negative_rows, 'num_relation_dependencies')}`")
        lines.append(f"- `dep_per_rule` 中位数：`{_median_text(positive_rows, 'dep_per_rule')}`，负增益为 `{_median_text(negative_rows, 'dep_per_rule')}`")
        lines.append("")
        ...
        lines.append("按最终被选中的 stage 看，正增益 relation 更常落在 dependency stage：")
        lines.append("")
        lines.append(f"- 正增益 relation：`dependency = {pos_dep}`，`rule_only = {pos_rule}`")
        lines.append(f"- 负增益 relation：`dependency = {neg_dep}`，`rule_only = {neg_rule}`")
        lines.append("")
    ...
    lines.append("")
    lines.append("不同数据集上的 relation-level 增益分布差异明显，说明 dependency 的收益不仅取决于单个 relation 的局部结构，也取决于整个数据集的规则池与候选依赖边的质量。")
    lines.append("")
    ...
    lines.append("")
    lines.append("代表性正例并不一定是训练样本最多的 relation。更常见的模式是：baseline 已经有可用规则信号，但尚未饱和；同时规则数与 dependency 数达到一定规模，从而允许模型通过 rule interaction 做进一步修正。")
    lines.append("")
    lines.append("- baseline stage1 还没有饱和，但已经有一定 rule 信号")
    lines.append("- 往往拥有中等到偏高的 rule 数和 dependency 数")
    lines.append("- 很多例子最终还是选择了 `dependency` stage，而不是只靠更强的 stage1")
    lines.append("- 更适合被多条互补规则共同支持")
    lines.append("")
    ...
    lines.append("")
    lines.append("- 一类是 `FB15k-237` 上那种高规则数、高 dependency 数的 dense relation，dependency 更像是在已有 rule pool 上做强组合。")
    lines.append("- 另一类是 `hetionet: DrD` 这种规模并不大、但结构很明确的 relation，少量高质量 dependency 也能带来明显收益。")
    lines.append("")

    lines.append("## Representative Negative Relations")
    lines.append("")
    lines.append("负例通常对应两种风险：其一是 baseline 本身已经较强，额外 dependency 容易过修正；其二是 dependency 虽多，但质量不稳定，valid 侧的偏好无法稳定迁移到 test。")
    lines.append("")
...
def build_dataset_analysis_md(rows: List[Dict[str, object]]) -> str:
    ...
    lines.append("")
    lines.append("本表统计各数据集规模、rule 数量、dependency 数量，以及按 type 聚合后的 rule/dependency 结构。")
    lines.append("")
    lines.append("相关表格：")
    lines.append("")
    ...
    if rows:
        ...
        lines.append("")
        lines.append(f"- 规则最多的数据集：`{largest_rule['dataset']}`，共有 `{largest_rule['#rule']}` 条 rule。")
        lines.append(f"- filtered dependency 最密的数据集：`{largest_filtered['dataset']}`，`filtered_dep_per_rule = {fmt_float_short(largest_filtered['filtered_dep_per_rule'])}`。")
        lines.append(f"- `B` rule 占比最高：`{most_b['dataset']}`，`B_rule_ratio = {fmt_float_short(most_b['B_rule_ratio'])}`。")
        lines.append(f"- `Ud` rule 占比最高：`{most_ud['dataset']}`，`Ud_rule_ratio = {fmt_float_short(most_ud['Ud_rule_ratio'])}`。")
        lines.append("")

    lines.append("## Interpretation")
    lines.append("")
    lines.append("- `FB15k-237 / codex-m / codex-l / YAGO3-10` 这类数据集更像大规模 relation-wise rule aggregation 场景。")
    lines.append("- `KG20C / WN18RR` 关系数较少，但 rule 和 dependency 结构更紧凑，适合做受控比较。")
    lines.append("- `hetionet` 的 graph 规模和语义复杂度都高，通常更依赖更强的 structural bias。")
    lines.append("- `wikidata5m` 当前只有 rule application 结果，dependency / aggregation 统计仍为空，这一点在表格里会直接体现。")
    lines.append("")
    return "\n".join(lines)
...
def build_readme(report_dir: Path) -> str:
    ...
    lines.append("")
    lines.append("本目录包含三部分分析：")
    lines.append("")
    lines.append("1. 整体指标总表与 summary")
    lines.append("2. 数据集逐关系增益分析")
    lines.append("3. 数据集规模与 rule/dependency 统计")
    lines.append("")
    ...
    lines.append("")
    lines.append(f"生成时间目录：`{report_dir}`")
    lines.append("")
    return "\n".join(lines)
