...
MASTER_LOG="${LOG_ROOT}/hetionet_filter_to_step5_master.log"
mkdir -p "${LOG_ROOT}"
...
run_stage() {
    local stage="$1"
    shift
    local log_path="${LOG_ROOT}/hetionet_${stage}.log"
...
echo "[$(date '+%F %T')] Queue launcher started for hetionet filter->step5." | tee -a "${MASTER_LOG}"
cd "${ROOT_DIR}"
run_stage filter python filter_dependency.py -d hetionet --target_split valid --min_supp 5
