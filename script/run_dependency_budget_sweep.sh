...
canonical_running() {
    ...
    pgrep -f "bash /home/sy/RuleDep/script/run_lragg_baseline.sh .*" >/dev/null 2>&1 && return 0
    return 1
}
...
run_filter_sweep() {
    ...
    (
        cd "${ROOT_DIR}"
        "${PYTHON_BIN}" -u src/ruledep/filter_dependency.py \
            -d "${dataset}" \
...
run_step5_variant() {
    ...
    (
        ...
        "${PYTHON_BIN}" -u src/ruledep/aggregation.py \
            -d "${dataset}" \
