...
## Query-Candidate Co-Firing Frequency

The reviewer also asked for the number of rules firing per candidate and the
frequency with which three or more rules co-fire. This is an instance-level
statistic and is different from counting unique three-rule combinations.

We count every positive and negative query-candidate row in the relation-local
training datasets. For each row, the statistic is the number of unique fired
rule IDs retained by rule application. No `TOP_K=500` or minimum-evidence filter
is applied in this table; the underlying rule-application files retain at most
200 rules per candidate.

| Dataset | Query-candidates | Mean fired rules | Median | P90 | Query-candidates with >=3 rules | Fraction with >=3 rules |
|---|---:|---:|---:|---:|---:|---:|
| KG20C | 3,244,620 | 5.00 | 3 | 10 | 2,004,441 | 61.78% |
| WN18RR | 10,259,418 | 9.00 | 4 | 22 | 6,727,011 | 65.57% |
| Codex-M | 9,051,849 | 13.42 | 11 | 25 | 8,205,198 | 90.65% |
| FB15k-237 | 11,999,739 | 42.57 | 24 | 113 | 10,784,464 | 89.87% |
| Codex-L | 36,479,367 | 10.14 | 7 | 21 | 31,105,566 | 85.27% |
| YAGO3-10 | 32,182,552 | 9.42 | 6 | 21 | 24,838,995 | 77.18% |

Across all 103,217,545 query-candidate instances, the candidate-weighted mean is
13.70 fired rules and 83,665,675 instances (81.06%) contain at least three
co-fired rules. The pooled values are dominated by Codex-L and YAGO3-10, so the
dataset-level range is the more informative summary.

This result shows that three-rule co-firing itself is common. The higher-order
analysis below asks the separate question of whether those co-firings define a
reliably estimable effect beyond the constituent pairwise gains.
...
The instance-level statistics above rule out the explanation that pairwise
modeling is sufficient merely because three rules seldom co-fire. They co-fire
on 61.78%-90.65% of query-candidate instances. The comparison supports the
pairwise restriction for two different reasons.
...
> higher-order effects. At the query-candidate level, an average of
> 5.00-42.57 rules fire per candidate (median 3-24), and 61.78%-90.65% of
> candidates have at least three co-fired rules, so higher-order co-firing is
> not intrinsically rare. For each relation, we then considered up to 500
> eligible rules and enumerated observed rule pairs and three-rule
> combinations. A combination was support-qualified when its joint support met
> the same minimum threshold used in dependency mining. We classified a
...
The query-candidate co-firing statistics are generated by:

```bash
python src/reporting/analyze_candidate_rule_cofiring.py \
...
- `candidate_rule_cofiring_stats.csv`: dataset-level fired-rule count
  distribution and the number/fraction of query-candidate rows with at least
  two or at least three fired rules.
